...
def createJob = freeStyleJob(containerFolder + '/Nexus_Upload_Application')
...
createJob.with {
    ...
    steps {
        ...
        copyArtifacts('Archive_Application') {
            includePatterns('**/*')
...
echo "#####################################################################################"
echo "# ===============> UPLOADING APPLICATION BINARIES TO NEXUS <======================= #"
echo "#####################################################################################"
...
tag=${NEXUS_TAG} nexus_user=${NEXUS_USERNAME} nexus_password=${NEXUS_PASSWORD} backup_dir=${BACKUP_DIRECTORY}" --tags "app_upload" site.yml -vvv
